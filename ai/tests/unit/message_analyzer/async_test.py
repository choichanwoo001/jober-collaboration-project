...
# 모킹 API
async def classify_message_type(userMessage: str):
    """메시지 타입 유형 모킹 메서드"""
    await asyncio.sleep(1.5)  # API 응답 1.5초 걸린다고 가정
    return {"type": "예약취소"}

async def classify_message_category(userMessage: str, category_main: str, category_sub_list: list):
    """메시지 타입 유형 모킹 메서드"""
    await asyncio.sleep(2.0)  # API 응답 2초 걸린다고 가정
    return {"category_sub": "구매취소"}

async def extract_message_fields(userMessage: str, fields_hint: list):
    """메시지 타입 유형 모킹 메서드"""
    await asyncio.sleep(0.8)  # 필드 추출 0.8초 걸린다고 가정
    return {"fields": {"주문번호": "12345"}}

# 순차 실행 테스트와 병렬 실행 테스트 비교
async def sequential_analyze(userMessage: str, category_main: str, category_sub_list: list):
    """순차 실행 테스트"""
    start = time.perf_counter()
    type_result = await classify_message_type(userMessage)
    category_result = await classify_message_category(userMessage, category_main, category_sub_list)
    extract_result = await extract_message_fields(userMessage, [])
...
async def parallel_analyze(userMessage: str, category_main: str, category_sub_list: list):
    """병렬 실행 테스트"""
    start = time.perf_counter()
    type_result, category_result = await asyncio.gather(
        classify_message_type(userMessage),
        classify_message_category(userMessage, category_main, category_sub_list)
    )
    extract_result = await extract_message_fields(userMessage, [])
    combined = {**type_result, **category_result, **extract_result}
    end = time.perf_counter()
...
async def test_sequential_analyze():
    """순차 실행 성능 테스트"""
    userMessage = "예약을 취소하고 싶어요"
    category_main = "주문"
    category_sub_list = ["예약취소", "환불요청"]
    
    result = await sequential_analyze(userMessage, category_main, category_sub_list)
...
async def test_parallel_analyze():
    ...
    result = await parallel_analyze(userMessage, category_main, category_sub_list)
...
async def test_parallel_vs_sequential_performance():
    ...
    # 순차 실행 시간 측정
    start = time.perf_counter()
    await sequential_analyze(userMessage, category_main, category_sub_list)
    sequential_time = time.perf_counter() - start
    
    # 병렬 실행 시간 측정
    start = time.perf_counter()
    await parallel_analyze(userMessage, category_main, category_sub_list)
