...
// ============================================
// 모든 키워드는 동적으로 추출되어 처리됩니다
// ============================================

// ============================================
// 내부 메시지/지시사항 감지 패턴 (완전 범용)
// ============================================

/**
 * 내부 메시지 감지용 정규식 패턴
 * 특정 키워드에 의존하지 않고 언어 구조와 패턴으로 판별
 */
...
export const EXPLANATORY_TEXT_PATTERNS = [
  // === 작업 동사 명령형 패턴 (ACTION_VERBS 대체) ===
  // 삭제, 제거, 변경, 수정, 교체, 대체, 없애, 빼, 추가, 넣어, 포함, 삽입, 바꾸, 변환, 전환, 확인, 체크, 검토
  /[^.]*[를을]\s*(?:삭제|제거|변경|수정|교체|대체|없애|빼|추가|넣어|포함|삽입|바꾸|변환|전환|확인|체크|검토)(?:하고|합니다|해야|하세요|세요|해주세요)[^.]*\.\s*/g,
  /[^.]*로\s*(?:변경|수정|교체|대체|변환|전환)(?:합니다|해야|하세요|세요|해주세요)[^.]*\.\s*/g,
  
  // === 과정/절차 설명 패턴 ===
  /(?:변경|수정|삭제|추가|교체|대체)(?:될|되어야\s*할|해야\s*할)\s*부분[^.]*\.\s*/g,
  /실제\s*적용되어야\s*할[^.]*\.\s*/g,
  /(?:처리|작업|절차|단계|과정)\s*설명[^.]*\.\s*/g,
  
  // === 메타 키워드 패턴 (META_KEYWORDS 대체) ===
  // 문구, 내용, 텍스트, 표현, 문장, 단어, 부분, 항목
  /[^.]*라는\s*(?:문구|내용|텍스트|표현|문장|단어|부분|항목)[^.]*\.\s*/g,
  /(?:문구|내용|텍스트|표현|문장|단어|부분|항목)[가는이를을][^.]*\.\s*/g,
  // 과정, 단계, 절차, 작업, 처리
  /[^.]*(?:과정|단계|절차|작업|처리)[이가을를를][^.]*\.\s*/g,
  // 태그, 마크업, 코드, 미리보기, 화면, 페이지, 뷰
  /[^.]*(?:태그|마크업|코드|미리보기|화면|페이지|뷰)[나를을를에는][^.]*\.\s*/g,
  // 광고성, 홍보, 마케팅, 프로모션
  /[^.]*(?:광고성|홍보|마케팅|프로모션)[^.]*\.\s*/g,
  
  // === 대상자 3인칭 언급 패턴 ===
  /(?:고객|사용자|방문자|수신자)[이가을를]\s*(?:보는|보이는|받는|확인하는|접하는)[^.]*\.\s*/g,
  /(?:고객|사용자|방문자|수신자)[에게한테]\s*(?:보이는|전달되는|노출되는|제공되는)[^.]*\.\s*/g,
  
  // === 명령형/지시형 패턴 ===
  /[^.]*[를을]\s*[^.]*로\s*(?:바꾸|변경|수정|교체)(?:세요|하세요|해주세요)[^.]*\.\s*/g,
  
  // === 설명형 접속사로 시작하는 문장 패턴 (EXPLANATORY_CONNECTIVES 대체) ===
  // 먼저, 그다음, 그리고, 또한, 추가로, 더불어, 아울러
  /^(?:먼저|그다음|그리고|또한|추가로|더불어|아울러)[^.]*\.\s*/gm,
  // 따라서, 그러므로, 결과적으로, 최종적으로
  /^(?:따라서|그러므로|결과적으로|최종적으로)[^.]*\.\s*/gm,
  // 예를 들어, 예시로, 예컨대
  /^(?:예를\s*들어|예시로|예컨대)[^.]*\.\s*/gm,
  // 만약, 만약에, 경우에, 상황에
  /^(?:만약|만약에|경우에|상황에)[^.]*\.\s*/gm,
  // 원래, 기존의, 이전의, 과거의, 새로운, 다른
  /^(?:원래|기존의|이전의|과거의|새로운|다른)[^.]*\.\s*/gm,
  
  // === 지시대명사로 시작하는 설명 패턴 ===
  /^(?:이것은|저것은|그것은|이런|저런|그런)[^.]*\.\s*/gm,
  /(?:이\s*(?:내용|부분|텍스트|문장)|저\s*(?:내용|부분)|그\s*(?:내용|부분))[이가을를][^.]*\.\s*/g,
...
export function extractBulletKeywords(text: string): string[] {
  ...
  // 불릿 포인트 형태 키워드 추출 (• 키워드 형태)
  const simpleBulletPattern = /•\s*([가-힣]+)/g
  while ((match = simpleBulletPattern.exec(text)) !== null) {
    if (match[1] && match[1].length >= 2) {
      keywords.push(match[1].trim())
    }
  }
